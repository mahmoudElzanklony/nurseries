<?php
return [
    // ---------------------start email messages -------------------
    'activation_email_code'=>'تفعيل البريد الالكتروني الخاص بموقع sync',
    'activation_email_code_body_message'=>'لقد تمت عملية التسجيل بنجاح من فضلك قم بتفعيل الحساب الخاص بكم من خلال الضغط علي هذا الرابط ',
    // ---------------------end email messages -------------------

   // ----------------------start user table inputs ---------------------
    'username'=>'اسم المستخدم',
    'email'=>'البريد الالكتروني',
    'password'=>'كلمة السر',
    'city_id'=>'المدينة',
    'phone'=>'رقم الهاتف',
    'image'=>'الصورة',
   // ----------------------end user table inputs ---------------------


    // ----------------------start seller info  inputs ---------------------
    'name'=>'الاسم',
    'info'=>'التفاصيل',
    'type'=>'النوع',
    'business_email'=>'البريد الالكتروني الخاص بالعمل',
    'business_phone'=>'رقم الجوال الخاص بالعمل',
    'images'=>'الصور',
    'owner_name'=>'اسم  صاحب الحساب',
    'bank_name'=>'اسم البنك',
    'bank_account'=>'رقم الحساب',
    'bank_iban'=>'رقم ال iban',

    // ----------------------start packages table inputs ---------------------
    'no_points'=>'عدد النقاط',
    'price'=>'السعر',
    'expire_date'=>'انتاء الصلاحية',
    'package'=>'الباقة',
    // ----------------------end packages table inputs ---------------------


    // ----------------------start products table inputs ---------------------
    'ar_name'=>'الاسم بالعربي',
    'en_name'=>'الاسم بالانجليزي',
    'ar_description'=>'الوصف بالعبي',
    'en_description'=>'الوصف بالانجليزي',
    'plant_type'=>'نوع النبات',
    'main_price'=>'السعر الاساسي',
    'wholesale_prices'=>'اسعار الجملة',
    'answers'=>'الاجابات',
    'features'=>'المميزات',
    'discounts'=>'التخفيضات',
    'cares'=>'الرعايات',
    'care'=>'الرعاية',
    'time_number'=>'عدد المرات',
    'time_type'=>'نوع الوقت',
    'inner'=>'داخلي',
    'outer'=>'خارجي',
    // ----------------------end products table inputs ---------------------

    // ----------------------start bank model payment table inputs ---------------------
    'type_of_transfer'=>'نوع التحويل',
    'receiver'=>'المرسل له',
    'date_of_transfer'=>'تاريخ التحويل',
    'number_sent_from'=>'الرقم المحول منه',
    'number_sent_to'=>'الرقم المحول الية',
    'card_number'=>'رقم البطاقة',
    'end_date'=>'تاريخ الانتهاء ',
    'cvv'=>'رقم التأمين',
    // ----------------------start bank model payment table inputs ---------------------

    'money'=>'المبلغ المطلوب',


    // ----------------------start bank tickets- ---------------------
    'category'=>'القسم',
    'title'=>'العنوان',
    'message'=>'الرسالة',
    // ----------------------end bank tickets- ---------------------
    // ----------------------start of  rates- ---------------------
    'product'=>'المنتج',
    'comment'=>'التعليق',
    'rate_product_info'=>'تقييم المنتج كما هو بالموصف',
    'rate_product_services'=>'تقييم خدمات المنتج',
    'rate_product_delivery'=>'تقييم توصيل المنتج',
    // ----------------------end of  rates- ---------------------


    // ----------------------start of  orders- ---------------------
    'orders'=>'الطلبات',
    'preparing'=>'تجهيز الطلب',
    'delivered'=>'توصيل',
    'completed'=>'استلام',
    'cancelled'=>'تم الالغاء',
    'to_default_address'=>'الي عنوانك الافتراضي',
    'dont_support_delivery_product'=>'لا يدعم توصيل منتج ',
    // ----------------------end of  orders- ---------------------

    // ----------------------start of  custom orders----------------------
    'address'=>'العنوان',
    'seller'=>'البائع',
    'sellers'=>'البائعين',
    'days_delivery'=>'عدد الايام المستغرقة للتوصيل',
    'delivery_price'=>'سعر التوصيل',
    'not_support_delivery'=>'هذا المنتج لا يدعم التوصيل لمكانك',
    'accepted_offer'=>'تم قبول عرضك المقدم بنجاح الخاص بطلب ',
    'pending'=>'معلق',
    'active'=>'نشطة',
    'accepted'=>'تم الموافقة علية',
    'rejected'=>'تم الغاء الطلب',
    'visa'=>'بطاقة الائتمان',
    // ----------------------end of  custom orders----------------------


    // ----------------------start  tickets clients- ---------------------
    'make_ticket_from'=>'تم عمل تذكرة جديدة عن طريق ',
    'ticket'=>'تذكرة',
    'status'=>'الحالة',
    'ticket_replied_from'=>'تم اضافة رد جديد عن طريق ',
    // ----------------------end  tickets clients- ---------------------
    // ----------------------start  marketer - ---------------------
    'ask_to_take_profit'=>'تم طلب استلام الارباح من قبل المسوق ',
    'order_number'=>'بخصوص طلب شراء الباقة رقم ',
    // ----------------------end  marketer - ---------------------

    // ----------------------start of  cities - ---------------------
    'country'=>'الدولة',
    // ----------------------start en cities - ---------------------

    // ----------------------start of  coupons - ---------------------
    'value'=>'القيمة',
    'number'=>'عدد المرات',
    'using_once'=>'استخدام مره واحده ام لا',

    // ----------------------start en coupons - ---------------------


];
